...
void f(ll i,ll one,ll two){
    string s_=to_string(i)+'#'+to_string(one)+'#'+to_string(two);
    if(s.count(s_))
        return;

    s.insert(s_);
    if(i==n){
        ans=max(ans,lcm_ll(one,two));
        return;
    }

    ll a=__gcd(one,v[i].ff);
    ll b=__gcd(two,v[i].ss);

    f(i+1,a,b);

    ll a1=__gcd(one,v[i].ss);
    ll b1=__gcd(two,v[i].ff);
    f(i+1,a1,b1);
}
...
void solve() {
    ...
    if(n==1) ans=max(ans,lcm_ll(v[0].ff,v[0].ss));
    f(1,v[0].ff,v[0].ss);
    cout<<ans<<endl;
